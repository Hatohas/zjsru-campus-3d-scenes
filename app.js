...
function createMiddleRoad() {
    ...
    const middleRoad = new THREE.BoxGeometry(100, 0.3, 20);
...
function createWalls() {
    ...
    const southWall = createWallSegment(253, 6, -110);
    walls.push(southWall);
    scene.add(southWall);

    const northWall = createWallSegment(253, 6, 130);
    walls.push(northWall);
    scene.add(northWall);

    const westWallSouth = createWallSegmentVertical(6, -128, -105, 15);
    walls.push(westWallSouth);
    scene.add(westWallSouth);

    const westWallNorth = createWallSegmentVertical(6, -128, 25, 125);
    walls.push(westWallNorth);
    scene.add(westWallNorth);

    const eastWallSouth = createWallSegmentVertical(6, 128, -105, 15);
    walls.push(eastWallSouth);
    scene.add(eastWallSouth);

    const eastWallNorth = createWallSegmentVertical(6, 128, 25, 125);
    walls.push(eastWallNorth);
    scene.add(eastWallNorth);

    createGate(-127, -110, 'south');
    loadSouthGateGLB();
    createGate(127, -110, 'south');

    createGate(-127, 130, 'north');
    createGate(0, 130, 'north_main');
    createGate(127, 130, 'north');

    createGate(-128, 20, 'west');
    createGate(128, 20, 'east');
}
...
function loadSouthGateGLB() {
    gltfLoader.load(
        'models/xm.glb',
        function (gltf) {
            const gateModel = gltf.scene;
            
            gateModel.position.set(0, 0, -130);
            gateModel.scale.set(30, 30, 30);
            gateModel.rotation.y = Math.PI;
            
            gateModel.traverse(function (child) {
                if (child.isMesh) {
            ...
            walls.push(gateModel);
            scene.add(gateModel);
            console.log('GLB南门模型加载成功！');
        },
        function (xhr) {
...
function loadCampusGLB() {
    gltfLoader.load(
        ...
        function (gltf) {
            ...
            campusModel.rotation.y = THREE.MathUtils.degToRad(180);
